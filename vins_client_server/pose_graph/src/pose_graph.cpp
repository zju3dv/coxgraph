...
void PoseGraph::registerPub(ros::NodeHandle &n) {
  pub_keyframe_ = n.advertise<comm_msgs::keyframe>("vins_keyframe", 1000);
...
void PoseGraph::addKeyFrame(KeyFrame *cur_kf, bool flag_detect_loop) {
  ...
  if (loop_index != -1) {
    ...
    if (cur_kf->findConnection(old_kf)) {
      ...
      T_A_B.block(0, 0, 3, 3) = relative_q.toRotationMatrix();
      ...
      if (cur_kf->has_loop) {
        ...
        if (cur_kf->sequence > 0) {
          ...
          posegraph_visualization->add_loopedge(
              P0, connected_P + Vector3d(VISUALIZATION_SHIFT_X,
...
void PoseGraph::publishKeyFrame(KeyFrame *cur_kf) {
  ...
  odom.pose.pose.position.y = p_W_S(1);
  odom.pose.pose.position.z = p_W_S(2);
  odom.pose.pose.orientation.w = q_W_S.w();
  odom.pose.pose.orientation.x = q_W_S.x();
  ...
  cv::Mat combined_descriptor(
      cur_kf->window_descriptors_.rows + cur_kf->descriptors_.rows,
      cur_kf->window_descriptors_.cols, cur_kf->descriptors_.type());
  if (cur_kf->window_descriptors_.rows > 0) {
    cur_kf->window_descriptors_.copyTo(
        combined_descriptor.rowRange(0, cur_kf->window_descriptors_.rows));
  }
  if (cur_kf->descriptors_.rows > 0) {
    cur_kf->descriptors_.copyTo(combined_descriptor.rowRange(
        cur_kf->window_descriptors_.rows,
        cur_kf->window_descriptors_.rows + cur_kf->descriptors_.rows));
  }
  ...
  for (int i = 0; i < NUM_ODOM_CONNECTIONS; i++) {
    ...
    if ((*rit)->sequence == cur_kf->sequence) {
      keyframe_msg.connections.push_back((*rit)->index);
    }
    rit++;
  }
  ...
  for (size_t i = 0; i < cur_kf->window_keypoints.size(); ++i) {
    ...
    Eigen::Vector3d landmark(cur_kf->point_2d_norm[point_idx].x,
                             cur_kf->point_2d_norm[point_idx].y, 1.0);
