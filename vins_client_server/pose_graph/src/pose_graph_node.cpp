...
void process() {
  ...
  while (true) {
    ...
    brisk::BriskDescriptorExtractor brisk_extractor(
    ...
    if (pose_msg != NULL) {
      ...
      if ((T - last_t).norm() > SKIP_DIS) {
        ...
        KeyFrame *keyframe = new KeyFrame(
            ...
            point_2d_uv, point_2d_normal, point_id, sequence, brisk_extractor);
        m_process.lock();
        start_flag = 1;
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub_pose =
      n.subscribe(n.resolveName("keyframe_pose"), 2000, pose_callback);
  ros::Subscriber sub_extrinsic =
      n.subscribe(n.resolveName("extrinsic"), 2000, extrinsic_callback);
  ros::Subscriber sub_point =
      n.subscribe(n.resolveName("keyframe_point"), 2000, point_callback);
