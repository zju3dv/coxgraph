...
void loadFeatures(vector<vector<cv::Mat> > &features,
  ...
  cv::Ptr<cv::FeatureDetector> detector = new brisk::BriskFeatureDetector(
      60.0, 4, true);  // Parameters: threshold, octaves, suppressScaleNonmaxima
  cv::Ptr<cv::DescriptorExtractor> descriptorExtractor =
      new brisk::BriskDescriptorExtractor(
          true, true,
          brisk::BriskDescriptorExtractor::Version::
              briskV2);  // Parameters: rotationInvariant, scaleInvariant
...
void testVocCreation(const vector<vector<cv::Mat> > &features) {
  ...
  // BRISKVocabulary voc(
  // "/media/fabiola/hd2ext4/vocabularies/BRISK2_k10_L3_voc_6193.yml.gz" );
